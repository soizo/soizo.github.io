...
const Convolutes = {
    右倾: [0, -1, 0, -1, 2, 2, 0, -1, 0],
    左倾: [0, -1, 0, 3, 2, -2, 0, -1, 0],
    桑拿: [1 / 9, 1 / 9, 1 / 9, 1 / 9, 1 / 9, 1 / 9, 1 / 9, 1 / 9, 1 / 9],
    // '桑拿':[
...
const randName = (userNames) => {
    ...
    return userNames[randRange2(0, userNames.length - 1)]
        .replace(/\d\d\d\d/, (_) => randRange(0, 9999))
        .replace(/_/g, (_) => k);
};
...
const patina = (imageEl, _config, app) => {
    ...
    requestAnimationFrame((_) => {
        ...
        const watermark = (_) => {
            ...
            const watermarkPlan = getWatermarkPlan(
                shift,
                randRange2(0, _config.watermarkPlan)
            );
        ...
        if (
            ...
            if (_config.lightNoise) {
                ...
                for (let i = 0; i < pixelData.length; i += 4) {
                    pixelData[i] =
                        pixelData[i] +
                        (randRange(0, _config.lightNoise) - halt); // * (255 - pixelData[i])/255;
                }
            }
            if (_config.darkNoise) {
                ...
                for (let i = 0; i < pixelData.length; i += 4) {
                    pixelData[i] =
                        pixelData[i] +
                        ((randRange(0, _config.darkNoise) - halt) *
                            (255 - pixelData[i])) /
                            255;
                    //噪声在亮部不那么明显
                }
            ...
            if (_config.contrast !== 1) {
                for (let i = 0; i < pixelData.length; i += 4) {
                    pixelData[i] =
                        (pixelData[i] - 128) * _config.contrast + 128;
                }
            }
            ...
            if (_config.convoluteName) {
                pixel = convolute(pixel, Convolutes[_config.convoluteName]);
        ...
        const one = (_) => {
            ...
            const _src = canvas.toDataURL(
                "image/jpeg",
                _config.quality / 100 + Math.random() * 0.1
            );
            ...
            _imgEl.onload = (_) => {
                ...
                if (isPop) {
                    popCtx.drawImage(
                        ...
                        (Math.floor((i - 1) / _config.pop) * popHeight) /
                            _config.pop,
                ...
                if (i < _round) {
                    ...
                    if (isPop) {
                        app.output = popCanvas.toDataURL(
                            "image/jpeg",
                            _config.quality / 100 + Math.random() * 0.05
                        );
                    }
                }
            };
        ...
        _imgEl.onload = (_) => {
            ...
            ctx.drawImage(_imgEl, 0, 0, _width, _height);
