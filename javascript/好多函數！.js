...
function 鏈結定位(path) {
  fetch(path)
    ...
    .then((data) => {
      ...
      const hrefElements = document.querySelectorAll("[href-catbox]");
      hrefElements.forEach((element) => {
        const key = element.getAttribute("href-catbox");
      ...
      const srcElements = document.querySelectorAll("[src-catbox]");
      srcElements.forEach((element) => {
        const key = element.getAttribute("src-catbox");
        if (isValidHttpUrl(key)) {
          element.src = key;
        } else if (data[key]) {
      ...
      const allCatboxElements = document.querySelectorAll(
        "[href-catbox], [src-catbox]"
      );
      allCatboxElements.forEach((element) => {
        const hrefKey = element.getAttribute("href-catbox");
        const srcKey = element.getAttribute("src-catbox");
        if (hrefKey && !isValidHttpUrl(hrefKey) && data[hrefKey]) {
          element.setAttribute("href", data[hrefKey]);
        }
        if (srcKey && !isValidHttpUrl(srcKey) && data[srcKey]) {
          element.setAttribute("src", data[srcKey]);
        }
      });
    })
...
function replaceTextWithDictionary(text, dictionary) {
  ...
  const pattern = new RegExp(Object.keys(lowerCaseDictionary).join("|"), "gi");
...
function 轉換道曆(input, trans = true, 格式) {
  ...
  if (格式 === undefined) {
    ...
    輸出 = 格式.replace(
      ...
      function (match) {
        if (match === "[干支]") {
          return convert_Year_To_2_char_Chinese(DATE.getFullYear(), trans);
        }
        switch (match[0]) {
          case "Y": /// 年
            switch (match.length) {
          ...
          case "W":
            switch (match.length) {
              ...
              case 2:
                return trans ? 曜日曆法(週) + "曜" : (週 < 10 ? "0" : "") + 週;
              case 3:
                return trans ? 曜日曆法(週) + "曜日" : engdays[週];
              default:
                return trans
                  ? 曜日曆法(週) + "曜日"
                  : (週 < 10 * (match.length - 1)
                      ? "0".repeat(match.length - String(週).length)
                      : "") + 週;
            }
          case "w":
            if (trans) {
